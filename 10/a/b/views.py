...
def patientlistele(request):
    patients = Patient.objects.all()
    return render(request, "patient-listele.html", context={'patients': patients})
...
# bu kısımda filter fonksiyonu kullanılmış, hata verirse tüm modeli açıp tekrar yapabiliriz.
...
def patientSocialHistoryView(request, id):
    try:
        ...
        return redirect(f'/patient-social-history-create/{id}')
    return render(
...
def patientVitalSignView(request, id):
    try:
        ...
        return redirect(f'/patient-detail/{id}')
    return render(
